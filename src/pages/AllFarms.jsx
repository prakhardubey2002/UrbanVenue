  ...
  useEffect(() => {
    const fetchFarms = async () => {
      try {
        ...
        setAddresses([...new Set(farmData.map((farm) => farm.address))])
  ...
  const handleFind = () => {
    const filtered = farms.filter((farm) => {
      return (
        ...
        (selectedAddress ? farm.address.includes(selectedAddress) : true)
      )
    })
            ...
            <table className="min-w-full bg-white border border-gray-300">
              <thead>
                <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                  ...
                  <th className="py-3 px-6 text-left border border-gray-300">Address</th>
                </tr>
              </thead>
              <tbody className="text-gray-600 text-sm font-light">
                {filteredFarms.map((farm, index) => (
                  <tr key={index}>
                    <td className="py-3 px-6 border border-gray-300">{farm.state}</td>
                    <td className="py-3 px-6 border border-gray-300">{farm.place}</td>
                    <td className="py-3 px-6 border border-gray-300">{farm.farmName}</td>
                    <td className="py-3 px-6 border border-gray-300">
                      {farm.address.addressLine1}, {farm.address.addressLine2}, {farm.address.citySuburb}, {farm.address.zipCode}, {farm.address.country}
                    </td>
                  </tr>
                ))}
              </tbody>
